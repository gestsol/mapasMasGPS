<template>
  <div>
    ...
    >
        ...
        <CustomMarker :options="{ position: BusCoord, anchorPoint: 'BOTTOM_CENTER' }">
              <div  >
        ...
        <CustomMarker :options="{ position: selectedParada, anchorPoint: 'BOTTOM_CENTER' }">
              <div  >
                <span style="background: rgb(255, 0, 0); color: white;" ><b> Parada  </b></span>
...
export default {
  ...
  data() {
    return {
      ...
      tramaSource: 0,
      CoordActaul:{},
      mapCenter:this.BusCoord || { lat: -33.4132183, lng: -70.5406616 }
  ...
  methods: {
      ...
      socket.onmessage = (event) => {
        ...
        if (tramaGps.data.hasOwnProperty('gps')) {
          ...
          if (tramaGps.data.source_id === this.selectedSource) {
            ...
            //this.CoordActaul = tramaGps.data.gps.location;
   ...
   /* cambiarBusCoord() {
      this.$emit('cambiar-prop',this.CoordActaul);
    }
*/
  },
  mounted() {
